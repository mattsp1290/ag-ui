...
func main() {
	...
	for _, user := range users {
		history := user.Resolver.GetConflictHistory()
		if history != nil && history.GetTotalConflicts() > 0 {
			fmt.Printf("\n%s's conflict history:\n", user.Name)
			fmt.Printf("  Total conflicts: %d\n", history.GetTotalConflicts())
			fmt.Printf("  Resolved: %d\n", history.GetResolvedCount())
			
			// Show recent conflicts
			recent := history.GetRecentConflicts(5)
			for _, conflict := range recent {
				fmt.Printf("  - Path: %s, Severity: %v\n", conflict.Path, conflict.Severity)
			}
		}
	}
...
func (s *CollaborationSession) AddUser(user *User) {
	...
	user.Resolver = state.NewDefaultConflictResolver(
		state.WithResolutionStrategy(state.LastWriteWins),
		state.WithConflictHistory(100),
	)
