...
// LegacyValidationError represents a simple validation error for backward compatibility
...
// ==============================================================================
// These interfaces provide backward compatibility in a simplified manner.
// The complexity has been reduced while maintaining API stability.
// New code should use the focused interfaces above.

// LegacyEncoder provides backward compatibility for encoder-focused workflows
// Simplified to use composition of focused interfaces
type LegacyEncoder interface {
	Encoder
	ContentTypeProvider
	StreamingCapabilityProvider
	// CanStream provides backward compatibility (same as SupportsStreaming)
	CanStream() bool
}

// LegacyDecoder provides backward compatibility for decoder-focused workflows
// Simplified to use composition of focused interfaces
...
// LegacyStreamEncoder provides backward compatibility for streaming encoders
// Simplified interface hierarchy
type LegacyStreamEncoder interface {
	LegacyEncoder
	StreamEncoder
...
// LegacyStreamDecoder provides backward compatibility for streaming decoders
// Simplified interface hierarchy
type LegacyStreamDecoder interface {
	LegacyDecoder
	StreamDecoder
	// Unified session management methods
	StartStream(ctx context.Context, r io.Reader) error
...
// LegacyStreamCodec provides backward compatibility for the old StreamCodec interface
// Simplified to remove unnecessary complexity while maintaining compatibility
type LegacyStreamCodec interface {
	Codec
	StreamCodec

	// Legacy session management (simplified)
	StartEncoding(ctx context.Context, w io.Writer) error
	WriteEvent(ctx context.Context, event events.Event) error
	EndEncoding(ctx context.Context) error
	StartDecoding(ctx context.Context, r io.Reader) error
