...
// BenchmarkJSONPatch tests patch application performance using internal JSON Patch
func BenchmarkJSONPatch(b *testing.B) {
	...
	for _, tc := range patchSizes {
		b.Run(tc.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				// Create patch
				patches := make(JSONPatch, 0, tc.patchCount)
				for j := 0; j < tc.patchCount; j++ {
					patches = append(patches, JSONPatchOperation{
						Op:    JSONPatchOpReplace,
						Path:  fmt.Sprintf("/key_%d/value", j),
						Value: rand.Float64() * 1000,
					})
				}
...
func BenchmarkSubscriptions(b *testing.B) {
	...
	for _, tc := range subscriberCounts {
		b.Run(tc.name, func(b *testing.B) {
			...
			// Create subscribers using the correct API
			subscribers := make([]func(), 0, tc.count)
			for i := 0; i < tc.count; i++ {
				// Using the Subscribe method from store
...
// BenchmarkTransactionPerformance tests transaction performance
	...
	b.Run("BatchTransaction", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			...
			for j := 0; j < 10; j++ {
				patches = append(patches, JSONPatchOperation{
					Op:    JSONPatchOpReplace,
					Path:  fmt.Sprintf("/key_%d/value", (i*10+j)%1000),
					Value: rand.Float64(),
				})
			}
			_ = tx.Apply(patches)
