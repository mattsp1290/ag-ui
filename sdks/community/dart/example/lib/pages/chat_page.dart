...
class ChatPageState extends ChangeNotifier {
  ...
  ChatPageState({required this.endpoint}) : _service = AgUiService() {
    ...
    if (event is TextMessageStartEvent) {
      ...
      if (delta.isNotEmpty) {
        ...
        final stateMessages = _messages
            .where((m) =>
                m.type == ChatMessageType.system && m.content.startsWith('📊'))
            .toList();
  ...
  void sendMultimodal(String questionText) async {
    ...
    try {
      await for (final event
          in _service.sendMultimodalMessage(endpoint.path, parts)) {
        if (_disposed) break;
        _handleEvent(event);
      }
    } catch (e) {
