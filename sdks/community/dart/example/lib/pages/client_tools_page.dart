...
class ClientToolsPageState extends ChangeNotifier with AgUiEventHandling {
  ...
  String?
      _pendingApproval; // human-readable summary; non-null while awaiting decision
  ...
  Stream<BaseEvent> _run() => _service.run(
        ...
        extraQuery:
            isApproval && !_approvalGate ? {'approval': 'off'} : const {},
      );
    ...
    try {
      while (!_aborted && !disposed && _pendingCalls.isNotEmpty) {
        ...
        for (final call in calls) {
          final result =
              await _execute(call); // may await a user decision (approval)
        ...
        await for (final event in _run()) {
          if (disposed || _aborted) {
            return; // stop consuming a post-error stream
          }
          _handleEvent(event);
        }
      }
  ...
  Future<String> _executeWithApproval(
    ...
    return jsonEncode(
        {'approved': true, 'result': _performDemoAction(name, args)});
  }
